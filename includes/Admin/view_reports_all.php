...
if (isset($_GET['p']) && is_numeric($_GET['p'])) { // Already been determined.
	...
	$q = "SELECT COUNT(rep_id) FROM greports";
...
// Make the query:
$q = "SELECT rep_name, rep_cat,rep_userid,rep_stat, DATE_FORMAT(rep_date, '%M %d, %Y') AS dr, rep_id FROM greports ORDER BY rep_date ASC LIMIT $start, $display";		
...
<tr>
	<th align="center" colspan="8"> <b>References</b> </th>
</tr>
<tr>
	<th align="left"><b>ID</b></th>
	<th align="left"><b>Reference name</b></th>
...
if ($pages > 1) {
	...
	if ($current_page != 1) {
		echo '<a href="view_reports_page.php?s=' . ($start - $display) . '&p=' . $pages . '&sort=' . $sort . '">Previous</a> ';
	}
	
	// Make all the numbered pages:
	for ($i = 1; $i <= $pages; $i++) {
		if ($i != $current_page) {
			echo '<a href="view_reports_page.php?s=' . (($display * ($i - 1))) . '&p=' . $pages . '&sort=' . $sort . '">' . $i . '</a> ';
		} else {
	...
	if ($current_page != $pages) {
		echo '<a href="view_reports_page.php?s=' . ($start + $display) . '&p=' . $pages . '&sort=' . $sort . '">Next</a>';
	}
